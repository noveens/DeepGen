...
def get_dataset(dataset, data_path):
    if dataset == 'MNIST':
        ...
        mean = [0.5]
        std = [0.5]
        transform = transforms.Compose([transforms.ToTensor(), transforms.Pad(2), transforms.Normalize(mean=mean, std=std)])
    ...
    testloader = torch.utils.data.DataLoader(dst_test, batch_size=256, shuffle=False, num_workers=10)
...
def evaluate_synset(it_eval, net, images_train, labels_train, testloader, config):
    local_args = SimpleNamespace(   
        device = config.device, 
		lr_net = 0.005,
        epoch_eval_train = 400,
        batch_train = 256,
        dc_aug_param = config.dc_aug_param,
        dsa = None,
	)
    
    net = net.to(local_args.device)
    images_train = images_train.to(local_args.device)
    labels_train = labels_train.to(local_args.device)
    lr = float(local_args.lr_net)
    Epoch = int(local_args.epoch_eval_train)
    lr_schedule = [Epoch//2+1]
    optimizer = torch.optim.SGD(net.parameters(), lr=lr, momentum=0.9, weight_decay=0.0005)
    criterion = nn.CrossEntropyLoss().to(local_args.device)

    dst_train = TensorDataset(images_train, labels_train)
    trainloader = torch.utils.data.DataLoader(dst_train, batch_size=local_args.batch_train, shuffle=True, num_workers=0)

    start = time.time()
    for ep in range(Epoch+1):
        loss_train, acc_train = epoch('train', trainloader, net, optimizer, criterion, local_args, aug = True)
        if ep in lr_schedule:
            lr *= 0.1
            optimizer = torch.optim.SGD(net.parameters(), lr=lr, momentum=0.9, weight_decay=0.0005)

    time_train = time.time() - start
    loss_test, acc_test = epoch('test', testloader, net, optimizer, criterion, local_args, aug = False)
...
def embedding_func(embed, imgs, batchsize):  # embedding function
    num = imgs.shape[0]
    num_batch = num // batchsize
    feats = []
    for bc in range(num_batch):
        feat = embed(imgs[bc * batchsize: (bc + 1) * batchsize])
        feats.append(feat)
    num_rest = num - num_batch * batchsize
    if num_rest > 0:
        feat = embed(imgs[-batchsize:])[-num_rest:]  # for better BN mean/std estimation
        feats.append(feat)
