...
def unnormalize(images):
    ...
    # mean = torch.tensor([ 0.5 ] * channels)
    # std = torch.tensor([ 0.5 ] * channels)
    # unnormalize = T.Normalize((-mean / std).tolist(), (1.0 / std).tolist())

    # Remove MNIST padding
    if channels == 1: images = images[:, :, 2:-2, 2:-2]
    return images
...
#         train_dataset = torchvision.datasets.CIFAR10(args.data_path, train=True, download=True, transform=train_transforms) # no augmentation
#         val_dataset = torchvision.datasets.CIFAR10(args.data_path, train=False, download=True, transform=val_transforms)
...
#         train_dataset = torchvision.datasets.MNIST(args.data_path, train=True, download=True, transform=train_transforms) # no augmentation
#         val_dataset = torchvision.datasets.MNIST(args.data_path, train=False, download=True, transform=val_transforms)
...
def mk_folders(results_path, models_path):
